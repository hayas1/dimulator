...
### i cannot implement APSP algorithm... ###
class APSP1TokenNode(UndirectedNode):
    ...
    def make_aveilable_color(self, n):      # for animation
        self.apsp_color = list(APSP1TokenNode.CMAP(i/n) for i in range(n))
    ...
    def bfs_for_apsp(self, round, message_tuples):
        ...
        if self.bloadcasting and round>0:
...
def animate_all_pair_shortest_path_1token(n=15, frames=None):
    # make node
    nodes = list(APSP1TokenNode(id=i) for i in range(n))
...
def save_apsp_1token_as_gif(n=15, frames=None, dir='./out', file='apsp1t.gif'):
    ...
    ani, _ = animate_all_pair_shortest_path_1token(n=n, frames=frames)
